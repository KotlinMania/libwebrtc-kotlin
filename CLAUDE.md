# Claude Code Project Instructions — libwebrtc-kotlin

## Project Overview

This is **libwebrtc-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`libwebrtc`](https://crates.io/crates/libwebrtc).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/libwebrtc/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/libwebrtc>
